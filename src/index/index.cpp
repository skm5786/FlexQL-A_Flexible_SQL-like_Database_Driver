 ...
 *
 * PERF CHANGES:
 *   1. index_create_presized() — allocates capacity for expected_rows upfront,
 *      eliminating all resize operations for large tables.
 *   2. index_put_int() — fast path for INT/DECIMAL PKs that skips the
 *      CellValue union dispatch.  Called from row_insert() hot path.
 *   3. HASH_INITIAL_CAPACITY raised from 16 → 256 so small tables also
 *      undergo fewer early rehashes.
 */
#include <cstdlib>
#include <cstring>
#include <cstdint>
...
/* ── Fast int probe — used by index_put_int() ───────────────────────────────
 * Skips the CellValue dispatch for INT keys on the hot insert path.
 * probe_insert_int checks slot->key_type == INT and slot->key_int == key.
 * ─────────────────────────────────────────────────────────────────────────── */
static size_t probe_insert_int(const HashIndex *idx, int64_t key) {
    size_t h = hash_int(key, idx->capacity), cap = idx->capacity;
    size_t tomb = SIZE_MAX;
    for (size_t i = 0; i < cap; i++) {
        size_t pos = (h + i) & (cap - 1);
        HashSlot *s = &idx->slots[pos];
        if (s->state == SLOT_EMPTY) return tomb != SIZE_MAX ? tomb : pos;
        if (s->state == SLOT_TOMBSTONE && tomb == SIZE_MAX) tomb = pos;
        /* Match: occupied + correct type + correct key */
        if (s->state == SLOT_OCCUPIED &&
            (s->key_type == COL_TYPE_INT || s->key_type == COL_TYPE_DECIMAL ||
             s->key_type == COL_TYPE_DATETIME) &&
            s->key_int == key)
            return pos;
    }
    return tomb != SIZE_MAX ? tomb : SIZE_MAX;
...
/* ── Section E: next_power_of_two ───────────────────────────────────────────
 * Find the smallest power-of-2 >= n.
 * Used by index_create_presized() to compute the initial capacity.
 * ─────────────────────────────────────────────────────────────────────────── */
 ...
 *
 * WHY extern "C" HERE:
 *   index.h declares these functions inside  #ifdef __cplusplus extern "C" { }
 *   which gives them C linkage in every .cpp file that includes the header.
 *   The definitions below must carry the same linkage specifier, otherwise
 *   the compiler generates a C++-mangled symbol for the definition while
 *   every caller expects a C-mangled symbol — an "undefined reference" at
 *   link time (made visible by -flto which validates IR-level linkage).
 *
 *   Rule: if a function is declared extern "C" in a header, its definition
 *   in a .cpp file must also be inside an extern "C" block (or the header
 *   must be included before the definition, which implicitly applies the
 *   extern "C" from the header — but only when the header guard is active).
 *   Wrapping definitions explicitly is unambiguous and always correct.
 *
 *   Static helpers (hash_int, probe_find, resize, …) are NOT in this block
 *   because they are never declared in any header and have no external callers.
 * ─────────────────────────────────────────────────────────────────────────── */
extern "C" {
...
HashIndex *index_create_presized(ColumnType pk_type, size_t expected_rows) {
    /* PERF: Compute capacity = next_pow2(expected_rows / 0.7).
     * At expected_rows=10M: 10M / 0.7 = 14.3M → next_pow2 = 16M slots.
     * This means zero rehash operations during the entire 10M insert run.
     * Cost: one calloc of 16M × sizeof(HashSlot) bytes (HashSlot = 56 bytes
     * on 64-bit) = 896MB.  This is a significant memory cost but eliminates
     * the CPU cost of ~23 resize-copy operations that together move ~2.3GB
     * of data.  For the benchmark environment this trade-off is correct. */
    size_t min_cap = (expected_rows * HASH_LOAD_FACTOR_DEN) / HASH_LOAD_FACTOR_NUM + 1;
    size_t capacity = next_pow2(min_cap);
    if (capacity < HASH_INITIAL_CAPACITY) capacity = HASH_INITIAL_CAPACITY;

    HashIndex *idx = (HashIndex*)calloc(1, sizeof(HashIndex));
    if (!idx) return nullptr;
    idx->slots = (HashSlot*)calloc(capacity, sizeof(HashSlot));
    if (!idx->slots) {
        /* Fallback to default-sized index if presized allocation fails */
        free(idx);
        return index_create(pk_type);
    }
...
int index_put_int(HashIndex *idx, int64_t key_int, Row *row) {
    if (!idx) return -1;
    /* Resize check — same threshold as index_put */
    if ((idx->count + idx->tombstones + 1) * HASH_LOAD_FACTOR_DEN >=
         idx->capacity * HASH_LOAD_FACTOR_NUM)
        if (resize(idx) != 0) return -1;

    size_t pos = probe_insert_int(idx, key_int);
    if (pos == SIZE_MAX) return -1;
    ...
    s->state    = SLOT_OCCUPIED;
    s->key_type = idx->pk_type;   /* INT or DECIMAL */
    s->key_int  = key_int;
    s->row      = row;
    return 0;
}
