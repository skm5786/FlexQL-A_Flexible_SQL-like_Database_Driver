/**
 * network.cpp  —  Query Executor  (Fixed)
 *
 * FIX 1 — WAL PERSISTENCE FOR USER DATABASES:
 *   QUERY_CREATE_DB now calls wal_register_db(db_name) so that user-created
 *   databases (SCHOOL, MYDB, etc.) are registered in data/_registry and
 *   will be recovered by wal_recover() on server restart.
 *
 *   Previously only BENCH_<fd> session databases were registered (in
 *   server.cpp), meaning user databases created via "CREATE DATABASE X;"
 *   were never registered and thus never recovered after restart.
 *
 * FIX 2 — Fast INSERT path:
 *   The comment "fast path must be called from server.cpp BEFORE parser_parse()"
 *   is now correct — server.cpp handles the fast path.  The QUERY_INSERT case
 *   here is the fallback for column-list inserts and other non-simple patterns.
 */
#include <cstring>
#include <cstdlib>
#include <cstdio>
...
int executor_execute(DatabaseManager *mgr, Database **current_db,
    ...
    case QUERY_CREATE_DB: {
        ...
        /* FIX: Register user-created DBs in WAL registry so they
         * survive server restarts.  Previously only BENCH_<fd>
         * session DBs were registered (in server.cpp), causing user
         * databases to vanish after restart. */
