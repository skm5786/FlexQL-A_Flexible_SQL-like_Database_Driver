 ...
 *
 *   1. PRE-SIZED HASH INDEX
 *      table_create() now calls index_create_presized(10M) for tables that
 *      declare a PRIMARY KEY.  This eliminates all 23 rehash operations that
 *      occur when inserting 10M rows into a capacity-256 index.
 *      Each rehash at ~5M entries copies ~280MB of slot data — very expensive.
 *
 *   2. FAST INTEGER INDEX PATH (index_put_int)
 *      For INT and DECIMAL primary keys (which covers every TA benchmark table),
 *      row_insert() calls index_put_int() directly instead of going through the
 *      CellValue union dispatch in index_put().  This saves one level of
 *      branching and one memcpy per row on the hot path.
 *
 *   3. LAZY B+ TREE INSERTS
 ...
 *
 *   4. REDUCED LOCK SCOPE IN row_insert()
...
/* ── Expected rows for pre-sized hash index ─────────────────────────────── */
/* PERF: Pre-size hash index for the expected 10M-row TA benchmark table.
 * This value is deliberately conservative — any table with a PK gets a
 * hash pre-sized for HASH_PRESIZED_ROWS entries.  For small tables this
 * wastes some memory but the gain on large tables far outweighs the cost.
 * At 10M rows: 10M / 0.7 = 14.3M → next_pow2 = 16M slots × 56 bytes = 896MB.
 *
 * NOTE: If memory is a concern for small tables, we could check
 * expected_rows at CREATE TABLE time — but the TA benchmark doesn't do that
 * so we always pre-size. */
#define HASH_PRESIZED_ROWS  12000000UL  /* 12M → capacity=16M, headroom for 11.7M */
...
Table *table_create(Database *db, const char *name,
    ...
    /* PERF: Pre-size the hash index for the full expected row count so that
     * no rehash operations occur during bulk insert.  For 10M rows this
     * eliminates ~23 resize-copy operations that together move ~2.3GB of
     * slot data.  The cost is a larger upfront allocation. */
    t->pk_index=nullptr;
    if (t->pk_col>=0) {
        t->pk_index = index_create_presized(
            t->schema[t->pk_col].type, HASH_PRESIZED_ROWS);
        if (!t->pk_index) {
            pthread_mutex_unlock(&db->schema_lock);
...
int row_insert(Table *table,
    ...
    /* ── Convert string values to typed cells ──────────────────────────────
     * PERF: We allocate the cells array from the arena first (zero cost bump),
     * then convert each string value.  VARCHAR strings are also arena-copied
     * to avoid fragmented malloc() calls.  This is the primary hot path for
     ...
     *
     * PERF: For INT/DECIMAL PKs we extract the raw int64_t directly from
     * the cell to use with index_put_int() later — avoiding re-dispatching
     * through the CellValue union on the write path.
     */
    int64_t pk_int_val = 0;
    bool    pk_is_int  = false;

    if (table->pk_col>=0 && !cells[table->pk_col].is_null) {
        const CellValue *npk = &cells[table->pk_col];

        /* Record the raw int for the fast index_put_int path */
        if (npk->type == COL_TYPE_INT || npk->type == COL_TYPE_DECIMAL ||
            npk->type == COL_TYPE_DATETIME) {
            pk_is_int = true;
            if (npk->type == COL_TYPE_DECIMAL) {
                /* Treat decimal as int64 for the index — same as index.cpp does */
                memcpy(&pk_int_val, &npk->data.decimal_val, 8);
            } else {
                pk_int_val = npk->data.int_val;
            }
        }
    ...
    /* ── PERF: Atomic row_id outside the write lock ─────────────────────── */
    ...
    /* PERF: Use index_put_int for INT/DECIMAL PKs — skips CellValue dispatch.
     * For VARCHAR PKs, fall back to index_put which handles the string key. */
    if (table->pk_index && table->pk_col>=0 && !row->cells[table->pk_col].is_null) {
        if (pk_is_int) {
            index_put_int(table->pk_index, pk_int_val, row);
        } else {
            index_put(table->pk_index, &row->cells[table->pk_col], row);
        }
    }

    /* ── PERF: Lazy B+ tree inserts ─────────────────────────────────────────
     *
...
int table_scan(Table *table, const WhereClause *where,
    ...
    if (wcol>=0 &&
        ...
        /* ── Rebuild dirty tree before first range scan ─────────────────────
